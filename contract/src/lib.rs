...
#[near_bindgen]
impl Contract {
    pub fn new_book(&mut self, author_name: String, book_name: String, genre: Genre, rented: bool) {
        let book = Book {
    ...
    pub fn new_user(&mut self, user: String, dob: i64, rental_status: RentalStatus, accountNumber: AccountId) {
        let user = User {
    ...
    pub fn check_out(&mut self, book_id: i64, user_id: i64, rental_start_time: u64, rental_end_time: u64){
       let user = self.users.get((user_id - 1 ) as u64).expect("User not found");
       let mut book = self.books.get((book_id - 1) as u64).expect("User not found");
       ...
       match book.rented {
        ...
        false => {
            ...
            env::log(format!("Book '{}' has been checked out by user '{}' at {}.", book.book_name, user.user, rental_start_time).as_bytes());
